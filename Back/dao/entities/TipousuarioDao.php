...
$descripcionTipoUsuario=$tipousuario->getDescripcionTipoUsuario();
      ...
      try {
          $sql= "INSERT INTO `tipousuario`( `idTipoUsuario`, `descripcionTipoUsuario`)"
          ."VALUES ('$idTipoUsuario','$descripcionTipoUsuario')";
  ...
  public function select($tipousuario){
      ...
      try {
          $sql= "SELECT `idTipoUsuario`, `descripcionTipoUsuario`"
          ."FROM `tipousuario`"
          ."WHERE `idTipoUsuario`='$idTipoUsuario'";
          $data = $this->ejecutarConsulta($sql);
          for ($i=0; $i < count($data) ; $i++) {
          $tipousuario->setIdTipoUsuario($data[$i]['idTipoUsuario']);
          $tipousuario->setDescripcionTipoUsuario($data[$i]['descripcionTipoUsuario']);
...
$descripcionTipoUsuario=$tipousuario->getDescripcionTipoUsuario();
      ...
      try {
          $sql= "UPDATE `tipousuario` SET`idTipoUsuario`='$idTipoUsuario' ,`descripcionTipoUsuario`='$descripcionTipoUsuario' WHERE `idTipoUsuario`='$idTipoUsuario' ";
         return $this->insertarConsulta($sql);
      } catch (SQLException $e) {
          throw new Exception('Primary key is null');
      }
