...
$condominio_idCondominio=$rol->getCondominio_idCondominio()->getIdCondominio();

      try {
          $sql= "INSERT INTO `rol`( `idrol`, `usuarioRol`, `passwordRol`, `descripcion`, `condominio_idCondominio`)"
          ."VALUES ('$idrol','$usuarioRol','$passwordRol','$descripcion','$condominio_idCondominio')";
  ...
  public function select($rol){
      ...
      try {
          $sql= "SELECT `idrol`, `usuarioRol`, `passwordRol`, `descripcion`, `condominio_idCondominio`"
          ."FROM `rol`"
          ."WHERE `idrol`='$idrol'";
          $data = $this->ejecutarConsulta($sql);
          for ($i=0; $i < count($data) ; $i++) {
          $rol->setIdrol($data[$i]['idrol']);
          $rol->setUsuarioRol($data[$i]['usuarioRol']);
          $rol->setPasswordRol($data[$i]['passwordRol']);
          $rol->setDescripcion($data[$i]['descripcion']);
           $condominio = new Condominio();
           $condominio->setIdCondominio($data[$i]['condominio_idCondominio']);
           $rol->setCondominio_idCondominio($condominio);
...
$condominio_idCondominio=$rol->getCondominio_idCondominio()->getIdCondominio();
      ...
      try {
          $sql= "UPDATE `rol` SET`idrol`='$idrol' ,`usuarioRol`='$usuarioRol' ,`passwordRol`='$passwordRol' ,`descripcion`='$descripcion' ,`condominio_idCondominio`='$condominio_idCondominio' WHERE `idrol`='$idrol' ";
         return $this->insertarConsulta($sql);
      } catch (SQLException $e) {
          throw new Exception('Primary key is null');
      }
